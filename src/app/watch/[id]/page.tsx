...
import { getMovieById, getRelated, getMovieDesc } from "../movies";
import styles from "./detail.module.css";
import watchStyles from "../watch.module.css";

// Placeholder cast — shared demo data until real per-title casts are wired in.
const CAST = [
  { name: "Ava Bennett", role: "Lead Voice", img: "https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=200&auto=format&fit=crop" },
  { name: "Marcus Reid", role: "Companion", img: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?q=80&w=200&auto=format&fit=crop" },
  { name: "Sofia Nguyen", role: "The Elder", img: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=200&auto=format&fit=crop" },
  { name: "James Okoro", role: "The Chief", img: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?q=80&w=200&auto=format&fit=crop" },
  { name: "Elena Rossi", role: "Supporting", img: "https://images.unsplash.com/photo-1534528741775-53994a69daeb?q=80&w=200&auto=format&fit=crop" },
  { name: "David Kim", role: "Voice", img: "" },
  { name: "Priya Sharma", role: "Villager", img: "" },
  { name: "Noah Clarke", role: "Narrator", img: "https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?q=80&w=200&auto=format&fit=crop" },
];

const GENRE_POOL = ["Family", "Fantasy", "Comedy", "Adventure", "Animation", "Drama", "Musical", "Action", "Sci-Fi", "Mystery"];

function genresFor(id: string): string[] {
  const h = Array.from(id).reduce((acc, ch) => acc + ch.charCodeAt(0), 0);
  const picks = [
    GENRE_POOL[h % GENRE_POOL.length],
    GENRE_POOL[(h * 3 + 1) % GENRE_POOL.length],
    GENRE_POOL[(h * 7 + 2) % GENRE_POOL.length],
  ];
...
export default function MovieDetailPage() {
  ...
  const description = movie.description || getMovieDesc(movie.title);
  const genres = genresFor(movie.id);
  const related = getRelated(movie.id, 12);

  const toggleSaved = (id: string) => setSaved((prev) => ({ ...prev, [id]: !prev[id] }));

  const handleCardMouseEnter = (m: any, e: React.MouseEvent) => {
  ...
  return (
    <div className={styles.detailPage}>
      ...
      <section className={styles.backdrop}>
        <img src={movie.image} alt={movie.title} className={styles.backdropImg} />
      ...
      <section className={styles.sectionBlock}>
        ...
        <div className={styles.trailerCard}>
          <img src={movie.image} alt={`${movie.title} trailer`} className={styles.trailerThumb} />
          ...
          <div className={watchStyles.footerTop}>
            <div className={watchStyles.footerBrandCol}>
              <div className={watchStyles.footerLogo}>
                <span className={watchStyles.showText}>SHOW</span>
                <span className={watchStyles.tivaText}>TIVA</span>
              </div>
      ...
      {hoveredMovie && popoverPos && (
        ...
        >
          ...
          <div className={watchStyles.popoverBody}>
            ...
            <p className={watchStyles.popoverDescription}>{getMovieDesc(hoveredMovie.title)}</p>
